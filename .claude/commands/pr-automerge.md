...
- **Unresolved review threads** — `main`'s ruleset blocks auto-merge until every
  review thread is resolved, so `--auto` never completes on an open thread even
  with every check green. List them:

  ```bash
  gh api graphql -f query='query{repository(owner:"knight-owl-dev",name:"devops"){pullRequest(number:'"<number>"'){reviewThreads(first:100){nodes{id isResolved comments(first:1){nodes{author{login} path line body}}}}}}}' \
